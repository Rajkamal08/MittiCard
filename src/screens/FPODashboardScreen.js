    ...
    >
      ...
      >
        ...
        <View style={fc.body}>
          {/* Left */}
          <View style={fc.left}>
            <Text style={fc.farmName} numberOfLines={1}>
              {farm.farm_name || 'Farm'}
            </Text>
            <Text style={fc.farmerName}>👤 {farm.farmer_name || '—'}</Text>
            {farm.farmer_phone && (
              <Text style={fc.phone}>📞 {farm.farmer_phone}</Text>
            )}
            <Text style={fc.meta}>
              {farm.crop
                ? `🌾 ${farm.crop.charAt(0).toUpperCase() + farm.crop.slice(1)}`
                : '🌾 No crop'}
          ...
          <View style={fc.right}>
            <View style={[fc.scoreBadge, { borderColor: scoreColor(s) }]}>
              ...
              <Text style={[fc.scoreNum, { color: scoreColor(s) }]}>
                {s || '—'}
              </Text>
              <Text style={fc.scoreLabel}>/100</Text>
            </View>
            {farm.size_acres && (
              <Text style={fc.acres}>{farm.size_acres} acres</Text>
            )}
          </View>
        </View>
        ...
        {/* Low nutrient chips */}
        <View style={fc.chips}>
          {farm.nitrogen != null && farm.nitrogen < 140 && (
            <View style={fc.lowChip}>
              <Text style={fc.lowChipText}>N↓</Text>
            </View>
          )}
          {farm.phosphorus != null && farm.phosphorus < 11 && (
            <View style={fc.lowChip}>
              <Text style={fc.lowChipText}>P↓</Text>
            </View>
          )}
          {farm.potassium != null && farm.potassium < 108 && (
            <View style={fc.lowChip}>
              <Text style={fc.lowChipText}>K↓</Text>
            </View>
          )}
          {farm.zinc != null && farm.zinc < 0.6 && (
            <View style={fc.lowChip}>
              <Text style={fc.lowChipText}>Zn↓</Text>
            </View>
          )}
          {farm.organic_carbon != null && farm.organic_carbon < 0.5 && (
            <View style={fc.lowChip}>
              <Text style={fc.lowChipText}>OC↓</Text>
            </View>
          )}
          {farm.ph != null && (farm.ph < 5.5 || farm.ph > 8.0) && (
            <View style={[fc.lowChip, { backgroundColor: '#FEF3C7' }]}>
              <Text style={[fc.lowChipText, { color: '#92400E' }]}>pH⚠</Text>
            </View>
          )}
        </View>
      </TouchableOpacity>
...
export default function FPODashboardScreen({ navigation }) {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setError(
        isHindi
          ? 'डेटा लोड नहीं हो सका। दोबारा कोशिश करें।'
          : 'Could not load FPO data. Please try again.',
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const addFarm = async farmId => {
    ...
    try {
      ...
      Alert.alert(
        '✅ Success',
        isHindi
          ? 'खेत आपके डैशबोर्ड में जोड़ा गया।'
          : 'Farm added to your dashboard.',
      );
      ...
      Alert.alert(
        'Error',
        isHindi ? 'खेत जोड़ने में विफल।' : 'Failed to add farm.',
      );
    } finally {
      setAddingId(null);
    }
    ...
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>
          {isHindi ? 'डेटा लोड हो रहा है…' : 'Loading FPO data…'}
        </Text>
      </View>
    );
  }
        ...
        <View style={styles.header}>
          ...
          <Animated.View style={{ opacity: headerFade }}>
            <Text style={styles.headerMeta}>
              {isHindi ? 'FPO प्रबंधक डैशबोर्ड' : 'FPO Manager Dashboard'}
            </Text>
            ...
            <View style={styles.statsRow}>
              <StatTile
                icon="🏡"
                value={farms.length}
                label={isHindi ? 'खेत' : 'Farms'}
                color={colors.primary}
              />
              <StatTile
                icon="📊"
                value={stats?.average_soil_health_score ?? '—'}
                label={isHindi ? 'औसत स्कोर' : 'Avg Score'}
                color={scoreColor(stats?.average_soil_health_score || 0)}
              />
              <StatTile
                icon="⚠️"
                value={Object.values(db).reduce(
                  (sum, v) => Math.max(sum, v.count || 0),
                  0,
                )}
                label={isHindi ? 'कमी' : 'Deficient'}
                color="#E74C3C"
              />
            </View>
          </Animated.View>
            ...
            <View style={styles.errorCard}>
              ...
              >
                <Text style={styles.retryText}>
                  {isHindi ? 'दोबारा कोशिश करें' : 'Retry'}
                </Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {stats && total > 0 && (
            <View style={[styles.card, shadows.sm]}>
              ...
              <Text style={styles.sectionSub}>
                {isHindi
                  ? `${total} खेतों के नवीनतम स्कैन के आधार पर`
                  : `Based on latest scan from ${total} farms`}
              </Text>
                ...
                Object.keys(stats.crop_distribution).length > 0 && (
                  <View style={styles.cropDist}>
                    <Text style={styles.distLabel}>
                      {isHindi ? 'फसल वितरण:' : 'Crop distribution:'}
                    </Text>
                    <View style={styles.cropChips}>
                      {Object.entries(stats.crop_distribution).map(
                        ([crop, cnt]) => (
              ...
              <View style={styles.defBars}>
                <DefBar
                  label={isHindi ? 'नाइट्रोजन कम' : 'Nitrogen Low'}
                  count={db.nitrogen_low?.count || 0}
                  total={total}
                  color="#E74C3C"
                />
                <DefBar
                  label={isHindi ? 'फॉस्फोरस कम' : 'Phosphorus Low'}
                  count={db.phosphorus_low?.count || 0}
                  total={total}
                  color="#E67E22"
                />
                <DefBar
                  label={isHindi ? 'पोटेशियम कम' : 'Potassium Low'}
                  count={db.potassium_low?.count || 0}
                  total={total}
                  color="#F1C40F"
                />
                <DefBar
                  label={isHindi ? 'OC कम' : 'Organic Carbon Low'}
                  count={db.organic_carbon_low?.count || 0}
                  total={total}
                  color="#9B59B6"
                />
                <DefBar
                  label={isHindi ? 'जिंक की कमी' : 'Zinc Deficient'}
                  count={db.zinc_deficient?.count || 0}
                  total={total}
                  color="#2980B9"
                />
                <DefBar
                  label={isHindi ? 'आयरन की कमी' : 'Iron Deficient'}
                  count={db.iron_deficient?.count || 0}
                  total={total}
                  color="#16A085"
                />
              </View>
          ...
          {filteredFarms.length === 0 ? (
            <View style={styles.emptyCard}>
              ...
              <Text style={styles.emptySub}>
                {farms.length === 0
                  ? isHindi
                    ? '+ जोड़ें बटन से किसान जोड़ें।'
                    : 'Tap + Add to add farmers from your district.'
                  : isHindi
                  ? 'खोज कम करें।'
                ...
                districtFarms.map(f => (
                  <View key={f.farm_id} style={styles.districtFarmRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.distFarmerName}>
                        👤 {f.farmer_name} · 📞 {f.farmer_phone || '—'}
                      </Text>
                      {f.district && (
                        <Text style={styles.distMeta}>📍 {f.district}</Text>
                      )}
                    </View>
                    {f.already_added ? (
